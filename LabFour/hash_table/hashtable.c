...
struct hashtable * hashtable_new(int size)
{
  ...
  result->size = size;
  result->table = malloc(size * sizeof(struct listset*));
...
{
  int location = hash_string(item, this->size);
  listset_add(&this->table[location], item);
}
...
{
  int location = hash_string(item, this->size);
  return listset_lookup(&this->table[location], item);
}
...
{
  int location = hash_string(item, this->size);
  return listset_remove(&this->table[location], item);
}
